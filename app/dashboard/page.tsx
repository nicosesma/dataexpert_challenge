      ...
      <main className="flex-1 px-8 py-8 max-w-5xl mx-auto w-full">
        ...
        {error && (
          <div className="rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-3 text-red-400 text-sm">
            <span>{error}</span>
            {error.toLowerCase().includes("not authorized") || error.toLowerCase().includes("refresh") ? (
              <a
                href="/api/auth/google"
                className="ml-3 underline text-rose-400 hover:text-rose-300 transition-colors"
              >
